...
The `==` operator is *not* comparing the Python string `"spongebob@unc.edu"` to the column object directly. Instead, the column object's overloaded `__eq__()` method intercepts the `==` call and returns a SQL comparison expression. **This is very important to understand! Comparisons against columns _do not_ result in a boolean value.** SQLModel will later translate the resulting object into a SQL equality expression string.
...
This composability makes it easy to build reusable query logic without sacrificing readability or type safety. **This is an important design pattern called a [fluent interface](https://en.wikipedia.org/wiki/Fluent_interface).** 
